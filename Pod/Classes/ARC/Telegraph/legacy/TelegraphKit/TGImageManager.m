...
static const char *imageManagerQueueName = "com.telegraph.imagemanagerqueue";

static dispatch_queue_t imageManagerQueue()
{
    ...
    {
        queue = dispatch_queue_create(imageManagerQueueName, 0);
        dispatch_queue_set_specific(queue, imageManagerQueueName, (void *)imageManagerQueueName, NULL);
    });
    
    return queue;
}

static void dispatchOnImageManagerQueue(dispatch_block_t block, bool synchronous)
{
    if (block == NULL)
        return;
    
    if (dispatch_get_specific(imageManagerQueueName) == imageManagerQueueName)
        block();
    else
    {
        if (synchronous)
            dispatch_sync(imageManagerQueue(), block);
        else
            dispatch_async(imageManagerQueue(), block);
    }
}
