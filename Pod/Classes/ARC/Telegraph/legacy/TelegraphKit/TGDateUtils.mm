...
+ (NSString *)stringForTodayOrYesterday:(bool)today hours:(int)hours minutes:(int)minutes
{
    ...
    return [[NSString alloc] initWithFormat:today ? TGLocalizedStatic(@"Time.TodayAt") : TGLocalizedStatic(@"Time.YesterdayAt"), timeString];
}
...
+ (NSString *)stringForRelativeLastSeen:(int)date
{
    ...
    else
    {
        ...
        int minutesDiff = (t_now - date) / 60;
        int hoursDiff = (t_now - date) / (60 * 60);
