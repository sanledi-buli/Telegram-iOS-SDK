...
- (void)cancelItemsWithMessageId:(int)messageId
{
    if (messageId == 0)
        return;
    
    [self cancelItemsWithMessageIdsInArray:[[NSArray alloc] initWithObjects:[[NSNumber alloc] initWithInt:messageId], nil]];
}

- (void)cancelItemsWithMessageIdsInArray:(NSArray *)messageIds
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        [_itemsQueue enumerateKeysAndObjectsUsingBlock:^(NSString *path, TGDownloadItem *item, __unused BOOL *stop)
        {
            if (messageIdsSet.find(item.messageId) != messageIdsSet.end())
            {
                if (item.itemId != nil)
