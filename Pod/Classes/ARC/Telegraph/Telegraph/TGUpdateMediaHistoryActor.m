...
- (void)_requestMoreHistory
{
    [TGDatabaseInstance() loadLastRemoteMediaMessageIdInConversation:_peerId completion:^(int32_t messageId)
    {
        ...
        else
        {
            ...
            {
                _maxMid = messageId;
                self.cancelToken = [TGTelegraphInstance doRequestConversationMediaHistory:_peerId maxMid:messageId maxDate:0 limit:256 actor:(TGConversationMediaHistoryRequestActor *)self];
            }];
        }
    }];
}
